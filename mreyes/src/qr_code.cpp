...
static ros::Publisher booby_pub;
...
bool booby_found_status = false;

void rgb_callback(const sensor_msgs::ImageConstPtr& msg)
{
if(!booby_found_status)
{
  cv_bridge::CvImagePtr cv_image;
  cv_image = cv_bridge::toCvCopy(msg, "bgr8");
  cv::Mat &image = cv_image->image;

  Mat gray(image.size(), CV_MAKETYPE(cv_image->image.depth(), 1));
  Mat edges(image.size(), CV_MAKETYPE(cv_image->image.depth(), 1)); 
      ...
      if( top < contours.size() && right < contours.size() && bottom < contours.size() && contourArea(contours[top]) > 10 && contourArea(contours[right]) > 10 && contourArea(contours[bottom]) > 10 )
	{
	  ...
	  if(DBG==1)
		...
		circle( traces, Point(10,20) , 5 ,  Scalar(255,255,255), -1, 8, 0 );
	      ...
	      circle( traces, M[2], 2,  Scalar(255,0,255), -1, 8, 0 );
	      ...
	      float y_cor = 31 - 0.055 * (O[3].y + M[1].y) / 2;
	      float x_cor = 0.6 * ((O[3].x + M[1].x) / 2 - 360) / y_cor;

	      float theta = (L[0].y - M[1].y) / cv_distance(L[0],M[1]) / 0.32 * 45 / 180 * 3.1416;
	      x_cor = x_cor - 5 * sin(theta);
	      y_cor = y_cor + 5 * cos(theta);
	      cout << "x: " << x_cor << " y: " << y_cor << " theta: " << cos(theta) << endl; 
              geometry_msgs::Twist motor_msg;
              if(abs(x_cor) > 3)
                {
                    motor_msg.linear.x = 0.03;
                    motor_msg.angular.z = - 0.03 * x_cor;
                    motor_pub.publish(motor_msg);
                }
              if(abs(x_cor) < 3 && y_cor > 28)
                {
                    motor_msg.linear.x = 0.05;
              ...
              if(y_cor < 28)
                {    
                    ...
                    motor_msg.angular.z = 0;
                    motor_pub.publish(motor_msg);
                    geometry_msgs::PointStamped qr_msg;
                    qr_msg.header.frame_id = "base";
                    qr_msg.point.x = y_cor / 100;
                    qr_msg.point.y = - x_cor / 100;
                    qr_msg.point.z = 0.15;
                    booby_pub.publish(qr_msg);
                    booby_found_status = true;
                }
...
int main(int argc, char** argv)
{
  ...
  image_transport::Subscriber rgb_sub = it.subscribe("/camera/rgb/image_color", 100, rgb_callback);
  motor_pub = nh.advertise<geometry_msgs::Twist>("/motor_controller/twist",100);
  booby_pub = nh.advertise<geometry_msgs::PointStamped>("arm/boobie",1);
  ros::spin();
  return 0;
}
